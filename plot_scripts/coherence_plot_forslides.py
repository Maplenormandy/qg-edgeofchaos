...
fig = plt.figure(figsize=(8.7/2.54*0.5, 0.7*8.7/2.54), dpi=300)
...
for i in range(2):
    ...
    if i == 0:
        ...
        axr = fig.add_subplot(gs[i,0], sharex=axr)
    ...
    if i == 0:
        ...
        axa = fig.add_subplot(gs[i,0], sharex=axa)
    ...
    if i == 0:
        ...
        #axr.text(0.0, 1.03, '(a)', transform=axr.transAxes, ha='left', va='bottom')
        #axa.text(0.0, 1.03, '(b)', transform=axa.transAxes, ha='left', va='bottom')
        
        axa.xaxis.set_major_locator(mpl.ticker.MultipleLocator(base=2.0))
...
#plt.savefig('coherence_plot.pdf', dpi=300)
